...
# default alpha shape alpha values
alphas = [0.03]
...
def main():
    global pc_name, original_pc, used_pc, debug
    global voxel_size_value, nb_neighbors_value, std_ratio_value, nb_points_value, radius_value, use_tetgen
    global z_min, z_max
    ...
    parser.add_argument("--normals", action="store_true", help="Print dimension names to find normals")
    parser.add_argument("--name", default="cleaned_pc", help="Output image filename, default = cleaned_pc")
    # possible arguments for this: all or multiple of: downsample, outliers, mesh, poisson
    parser.add_argument("--parts", nargs='+', default="all", help="Which functions should be executed (default=all, options=downsample, outliers, mesh, poisson, ball)")
    ...
    if "all" in parts or "mesh" in parts:
        alpha_shape(used_pc)
    ...
    if "all" in parts or "densify" in parts:
        densify_pc(pcd=used_pc)
...
def to_open3d_cloud(las):
    ...
    # except (KeyError, ValueError) as e:
    #     print("Normals not found in LAS file: %s", e)
    #     normals = None
...
def alpha_shape(pcd):
    ...
    for alpha in alphas:
        ...
        # why compute normals after save file???
...
def poisson_recon(pcd):
    global octree_depth, poisson_density_cutoff, pc_name
    ...
    with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm:
        ...
        # Step 2: Scale mesh to ~10 units in largest dimension
...
def densify_pc(pcd):
    # (Optional) voxel downsample to regularize spacing
    # pcd = pcd.voxel_down_sample(voxel_size=0.05)

    # Estimate normals (needed for surface ops)
    # pc.estimate_normals()

    # # Step 1: Build a coarse mesh (e.g., via Poisson or Alpha Shape)
    # mesh, _ = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(pc, depth=6)

    # # Step 2: Sample new points uniformly on the mesh surface
    # pcd_dense = mesh.sample_points_uniformly(number_of_points=50000)

    # points = np.asarray(pc.points)
    # u, s, vh = np.linalg.svd(points - np.mean(points, axis=0))
    # print("Singular values:", s)
    # result: not near zero! Singular values: [7396.79796327 6588.11241688 1420.20828388]

    # o3d.visualization.draw_geometries([pcd_dense])
    pcd.estimate_normals()
    # methods for trying to solve error "invalid tetra mesh"
    #pc.remove_duplicated_points()
    pc = pcd.remove_statistical_outlier(nb_neighbors=20, std_ratio=2.0)[0]
    alpha = 0.5
    mesh_alpha = o3d.geometry.TriangleMesh.create_from_point_cloud_alpha_shape(pc, alpha)
    pcd_dense = mesh_alpha.sample_points_uniformly(number_of_points=30000)

    # Step 2: Resample dense points from alpha mesh
    pcd_dense = mesh_alpha.sample_points_uniformly(number_of_points=30000)
    pcd_dense.estimate_normals()

    # Step 3: Run Ball Pivoting Meshing
    radii = [0.05, 0.1, 0.2]  # choose based on scale of your data
